...
export default function LandingPage() {
    ...
    return (
        <div className="min-h-screen w-full max-w-full overflow-x-hidden bg-[var(--mutqan-bg)] font-cairo text-[#0b1320]" dir={isRTL ? 'rtl' : 'ltr'}>
...
function HeroSection() {
    ...
    return (
        <section className="relative isolate overflow-hidden bg-[var(--mutqan-bg)] px-4 pb-20 pt-32 sm:px-6 sm:pb-24 sm:pt-40 lg:px-8">
            <PaperTexture />
            <div className="pointer-events-none absolute inset-x-0 top-0 h-px bg-[#0b1320]/10" />

            <div className="relative mx-auto grid max-w-7xl gap-10 lg:min-h-[640px] lg:grid-cols-[minmax(0,0.96fr)_minmax(460px,1.04fr)] lg:items-center">
                <motion.div initial="hidden" animate="visible" variants={stagger} className="max-w-4xl">
                    <motion.p
                        variants={rise}
                        className="inline-flex w-fit rounded-full border border-[#00b2a9]/25 bg-white/80 px-4 py-2 text-xs font-black uppercase text-[var(--mutqan-accent)] shadow-card"
                    >
                    ...
                    <motion.h1
                        variants={rise}
                        className="mt-7 max-w-[820px] text-balance text-[clamp(2.25rem,4.55vw,3.75rem)] font-black leading-[1.28] tracking-normal text-[#0b1320] sm:leading-[1.26] lg:max-w-[760px]"
                    >
                        {t('landing.hero.headlineTop')}
                        <span className="mt-2 block text-[var(--mutqan-accent)] sm:mt-3">{t('landing.hero.headlineAccent')}</span>
                    </motion.h1>

                    <motion.p variants={rise} className="mt-7 max-w-3xl text-base font-bold leading-8 text-[#0b1320] sm:text-lg sm:leading-9">
                    ...
                    <motion.div variants={rise} className="mt-8 grid gap-3 sm:grid-cols-3">
                        {highlights.map((point) => (
                            <div
                                key={point}
                                className="flex min-h-[72px] items-start gap-3 rounded-lg border border-[#0b1320]/10 bg-white/78 p-3.5 text-sm font-black leading-6 text-[#0b1320] shadow-card"
                            >
                                <CheckCircle2 className="mt-1 h-4 w-4 shrink-0 text-[var(--mutqan-accent)]" />
                                <span>{point}</span>
                            </div>
                        ))}
                    </motion.div>
...
function HeroPreview() {
    ...
    return (
        <div className="relative mx-auto max-w-xl">
            <div className="absolute -inset-4 rounded-lg border border-[#0b1320]/10 bg-white/32" />
            <div className="relative overflow-hidden rounded-lg border border-[#c9d4d0] bg-white shadow-[0_34px_100px_-58px_rgba(15,23,42,0.64)]">
                <div className="flex items-center justify-between border-b border-slate-200 bg-[var(--mutqan-surface-raised)] px-4 py-3">
                    <div className="flex items-center gap-2">
                        <span className="h-2.5 w-2.5 rounded-full bg-[#0b1320]/28" />
                        <span className="h-2.5 w-2.5 rounded-full bg-[#00b2a9]/50" />
                        <span className="h-2.5 w-2.5 rounded-full bg-amber-400/55" />
                    </div>
                    <p className="text-sm font-black text-[#0b1320]">{t('landing.heroPreview.title')}</p>
                </div>

                <div className="grid lg:grid-cols-[168px_minmax(0,1fr)]">
                    <aside className="hidden border-e border-slate-200 bg-[var(--mutqan-surface-soft)] p-4 lg:block">
                            ...
                            return (
                                <div
                                    ...
                                    className={cn(
                                        ...
                                        index === 0 ? 'bg-[#0b1320] text-white' : 'text-slate-600'
                                    )}
                                >
                    ...
                    <div className="min-w-0 p-5">
                        <div className="rounded-lg bg-[#0b1320] p-5 text-white">
                            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                                <div>
                                    <p className="text-xs font-black uppercase text-[#7be4df]">{t('landing.heroPreview.eyebrow')}</p>
                        ...
                        <div className="mt-4 grid gap-3 sm:grid-cols-2">
                            {metrics.map(({ label, value }, index) => {
                                const Icon = previewNavIcons[index] ?? ClipboardList
                                return (
                                    <div key={label} className="rounded-lg border border-slate-200 bg-[var(--mutqan-surface-raised)] p-4">
                                        <div className="flex items-center justify-between gap-3">
                                            <p className="text-sm font-bold text-slate-500">{label}</p>
                                            <Icon className="h-4 w-4 text-[var(--mutqan-accent)]" />
                                        </div>
                                        <p className="mt-3 text-2xl font-black text-[#0b1320]">{value}</p>
                                    </div>
                                )
                            })}
                        </div>
                        ...
                        <div className="mt-4 rounded-lg border border-slate-200 bg-white p-4">
                            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                                <div>
                                    <p className="text-sm font-black text-[#0b1320]">{t('landing.heroPreview.recordTitle')}</p>
                                    <p className="mt-1 text-xs font-semibold leading-5 text-slate-500">{t('landing.heroPreview.recordMeta')}</p>
                                </div>
                                <span className="inline-flex w-fit rounded-lg border border-[#00b2a9]/25 bg-[#00b2a9]/10 px-3 py-1.5 text-xs font-black text-[var(--mutqan-accent)]">
                            ...
                            <div className="mt-4 h-2 overflow-hidden rounded-full bg-[#f1ede2]">
                                <motion.span
                                    className="block h-full w-2/3 rounded-full bg-[var(--mutqan-accent)]"
                                    initial={{ width: '18%' }}
                                    animate={{ width: '68%' }}
                            ...
                            <div className="mt-4 grid gap-2 sm:grid-cols-4">
                                {steps.slice(1, 5).map(({ title }, index) => {
                                    ...
                                    return (
                                        <div key={title} className="relative rounded-lg bg-[var(--mutqan-surface-soft)] px-3 py-2">
                                            {index === 2 && (
                                                <motion.span
                                                    className="absolute top-2 h-2 w-2 rounded-full bg-[var(--mutqan-accent)]"
                                                    animate={isRTL ? { x: [0, -8, 0] } : { x: [0, 8, 0] }}
                                                    transition={{ duration: 1.8, repeat: Infinity, ease: 'easeInOut' }}
                                                />
                                            )}
                                            <Icon className="mb-2 h-4 w-4 text-[var(--mutqan-accent)]" />
                                            <p className="text-xs font-black text-[#0b1320]">{title}</p>
                                        </div>
                                    )
                                })}
                            </div>
                ...
                {items.map((item, index) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            <p className="text-sm font-black leading-6 text-[#0b1320]">{item}</p>
                        </motion.div>
                    )
                })}
            </div>
        ...
        <section className="bg-white px-4 py-24 sm:px-6 lg:px-8">
            <div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-[0.86fr_1.14fr] lg:items-start">
                ...
                <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-90px' }} variants={stagger} className="space-y-4">
                    <motion.div variants={rise} className="rounded-lg border border-[#0b1320]/12 bg-[#0b1320] p-6 text-white">
                    ...
                    <div className="grid gap-4 sm:grid-cols-2">
                        {items.map(({ title, body }, index) => {
                            ...
                            return (
                                <motion.div key={title} variants={rise} className="rounded-lg border border-slate-200 bg-[var(--mutqan-surface-raised)] p-5">
                                    <div className="mb-4 flex items-center justify-between gap-3">
                                        <span className="flex h-11 w-11 items-center justify-center rounded-lg bg-white text-[var(--mutqan-accent)] shadow-card">
                                            <Icon className="h-5 w-5" />
                                        </span>
                                        <span className="text-xs font-black text-slate-400">{String(index + 1).padStart(2, '0')}</span>
                                    </div>
                                    <h3 className="text-lg font-black leading-7 text-[#0b1320]">{title}</h3>
...
function ValuePillarsSection() {
    ...
    return (
        <section className="relative overflow-hidden bg-[var(--mutqan-bg)] px-4 py-24 sm:px-6 lg:px-8">
            <PaperTexture />
                    ...
                    {pillars.slice(0, 5).map((pillar, index) => {
                        ...
                        return (
                            <motion.div key={pillar.title} variants={rise} className="rounded-lg border border-[#d8ded8] bg-white p-5 shadow-card">
                                <span className="mb-6 flex h-12 w-12 items-center justify-center rounded-lg bg-[#00b2a9]/10 text-[var(--mutqan-accent)]">
                                    <Icon className="h-5 w-5" />
                                </span>
                                <h3 className="text-xl font-black leading-8 text-[#0b1320]">{pillar.title}</h3>
...
function ConnectedWorkflowSection() {
    ...
    return (
        <section id="workflow" className="relative overflow-hidden bg-[#0b1320] px-4 py-24 text-white sm:px-6 lg:px-8">
            <InkTexture />
            <div className="relative mx-auto max-w-7xl">
                <SectionIntro
                ...
                >
                    <div className="grid md:grid-cols-3 xl:grid-cols-9">
                        {steps.map(({ title, body }, index) => {
                            const Icon = flowIcons[index] ?? ClipboardList
                            return (
                                <motion.div key={title} variants={rise} className="relative min-h-[178px] border-b border-white/10 p-5 md:border-e xl:border-b-0">
                                    <div className="mb-5 flex items-center justify-between gap-3">
                                        <span className="flex h-11 w-11 items-center justify-center rounded-lg border border-white/10 bg-[#00b2a9]/12 text-[#7be4df]">
                                            <Icon className="h-5 w-5" />
                                        </span>
                                    ...
                                    {index < steps.length - 1 && (
                                        <span className="absolute bottom-5 hidden items-center text-[#00b2a9]/70 xl:flex ltr:right-[-13px] rtl:left-[-13px]">
                ...
                <div className="mt-8 h-px overflow-hidden rounded-full bg-white/10">
                    <motion.div
                        className="h-full bg-[var(--mutqan-accent)]"
                        initial={{ width: '0%' }}
        ...
        <section className="bg-white px-4 py-24 sm:px-6 lg:px-8">
            <div className="mx-auto max-w-7xl">
                ...
                >
                    <div className="flex items-center justify-between border-b border-slate-200 bg-[var(--mutqan-surface-raised)] px-4 py-3">
                        <div className="flex items-center gap-2">
                            <span className="h-2.5 w-2.5 rounded-full bg-[#0b1320]/30" />
                            <span className="h-2.5 w-2.5 rounded-full bg-[#00b2a9]/45" />
                            <span className="h-2.5 w-2.5 rounded-full bg-amber-400/55" />
                        </div>
                        <p className="text-sm font-black text-[#0b1320]">{t('landing.preview.windowTitle')}</p>
                    </div>

                    <div className="grid lg:min-h-[520px] lg:grid-cols-[200px_minmax(0,1fr)] xl:grid-cols-[220px_minmax(0,1fr)_280px]">
                        <aside className={cn('hidden bg-[var(--mutqan-surface-soft)] p-4 lg:block', isRTL ? 'border-l border-slate-200' : 'border-r border-slate-200')}>
                            <p className="mb-4 text-xs font-black uppercase text-slate-500">{t('landing.preview.navTitle')}</p>
                        ...
                        <div className="min-w-0 p-5 sm:p-6">
                            <div className="flex flex-col gap-4 border-b border-slate-200 pb-5 lg:flex-row lg:items-start lg:justify-between">
                                <div>
                                    <h2 className="text-2xl font-black leading-8 text-[#0b1320]">{t('landing.preview.boardTitle')}</h2>
                                    <p className="mt-2 max-w-xl text-sm font-semibold leading-6 text-slate-500">{t('landing.preview.boardBody')}</p>
                                </div>
                                <span className="inline-flex w-fit items-center gap-2 rounded-lg border border-[#00b2a9]/25 bg-[#00b2a9]/10 px-3 py-2 text-sm font-black text-[var(--mutqan-accent)]">
                                    <ShieldCheck className="h-4 w-4" />
                            ...
                            <div className="mt-5 grid gap-3 sm:grid-cols-3">
                                {metrics.slice(0, 3).map(({ label, value }, index) => {
                                    const Icon = previewNavIcons[index] ?? ClipboardList
                                    return (
                                        <div key={label} className="rounded-lg border border-slate-200 bg-[var(--mutqan-surface-raised)] p-4">
                                            <div className="flex items-center justify-between gap-3">
                            ...
                            <div className="mt-6 overflow-hidden rounded-lg border border-slate-200">
                                <div className="grid grid-cols-[0.8fr_1.6fr] gap-3 bg-[var(--mutqan-surface-soft)] px-4 py-3 text-xs font-black uppercase text-slate-500 sm:grid-cols-[0.8fr_1.7fr_1fr_1fr]">
                                ...
                                {rows.map((row) => (
                                    <div key={`${row.code}-${row.title}`} className="grid gap-3 border-t border-slate-200 bg-white px-4 py-4 sm:grid-cols-[0.8fr_1.7fr_1fr_1fr]">
                                        <div>
                                            <p className="text-xs font-black uppercase text-[var(--mutqan-accent)]">{row.code}</p>
                                        </div>
                                        <div className="min-w-0">
                                            <h3 className="text-base font-black leading-7 text-[#0b1320]">{row.title}</h3>
                                            <p className="mt-1 text-xs font-semibold leading-5 text-slate-500">
                                                {row.location} | {row.asset}
                                            </p>
                                        </div>
                                        <div className="flex items-start sm:block">
                                            <span className="inline-flex rounded-lg border border-[#00b2a9]/25 bg-[#00b2a9]/10 px-2.5 py-1.5 text-xs font-black text-[var(--mutqan-accent)]">
                                                {row.status}
                                            </span>
                                        </div>
                                        <div className="flex items-start sm:block">
                                            <span className="inline-flex rounded-lg border border-slate-200 bg-[var(--mutqan-surface-raised)] px-2.5 py-1.5 text-xs font-black text-slate-500">
                                                {row.owner}
                                            </span>
                                        </div>
                                    </div>
                        ...
                        <aside className={cn('hidden border-t border-slate-200 bg-[var(--mutqan-surface-raised)] p-5 xl:block xl:border-t-0 xl:p-6', isRTL ? 'xl:border-r' : 'xl:border-l')}>
                            <div className="flex items-center justify-between gap-3">
                                <p className="text-xs font-black uppercase text-slate-500">{t('landing.preview.badge')}</p>
                                <motion.span
                                    className="h-2 w-2 rounded-full bg-[var(--mutqan-accent)]"
                                    animate={{ opacity: [0.4, 1, 0.4], scale: [1, 1.18, 1] }}
                                    transition={{ duration: 1.6, repeat: Infinity, ease: 'easeInOut' }}
                                />
                            </div>
                            <div className="mt-5 space-y-3">
                                {details.slice(0, 2).map(({ title, body }, index) => {
                                    const Icon = previewNavIcons[index] ?? FileText
                                    return (
                                        <div key={title} className="rounded-lg border border-slate-200 bg-white p-4">
                                            <Icon className="mb-3 h-5 w-5 text-[var(--mutqan-accent)]" />
                                            <h3 className="font-black text-[#0b1320]">{title}</h3>
                    ...
                    {audiences.map((label, index) => {
                        ...
                        return (
                            <motion.div key={label} variants={rise} className="min-h-[150px] rounded-lg border border-[#d8ded8] bg-white p-5 shadow-card">
                                <Icon className="mb-5 h-6 w-6 text-[var(--mutqan-accent)]" />
                                <p className="text-base font-black leading-7 text-[#0b1320]">{label}</p>
                            </motion.div>
                        )
                    })}
                </motion.div>
                        ...
                        return (
                            <motion.div key={title} variants={rise} className="rounded-lg border border-slate-200 bg-[var(--mutqan-surface-raised)] p-5">
                                <div className="mb-4 flex items-center gap-3">
                                    <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-lg bg-white text-[var(--mutqan-accent)] shadow-card">
                                        <Icon className="h-5 w-5" />
                                    </span>
                                    <h3 className="text-lg font-black leading-7 text-[#0b1320]">{title}</h3>
                                </div>
...
function FinalCta() {
    ...
    return (
        <section className="relative overflow-hidden bg-[#0b1320] px-4 py-24 text-white sm:px-6 lg:px-8">
            <InkTexture />
            <motion.div
            ...
            >
                <p className="text-sm font-black uppercase text-[#7be4df]">{t('landing.finalCta.eyebrow')}</p>
...
function PrimaryCta() {
    ...
    return (
        <Link
            to="/contact"
            className="inline-flex h-12 w-full items-center justify-center gap-2 rounded-lg bg-[var(--mutqan-accent)] px-6 text-base font-black text-white shadow-[0_18px_44px_-30px_rgba(0,178,169,0.95)] transition-colors hover:bg-[#00968f] sm:w-auto"
        >
...
function SecondaryCta() {
    ...
    return (
        <a
            href="#workflow"
            className="inline-flex h-12 w-full items-center justify-center rounded-lg border border-[#0b1320]/18 bg-white px-6 text-base font-black text-[#0b1320] transition-colors hover:border-[#00b2a9]/45 hover:text-[var(--mutqan-accent)] sm:w-auto"
        >
...
function SectionIntro({
    ...
    return (
        ...
        >
            <motion.p variants={rise} className={cn('text-sm font-black uppercase', dark ? 'text-[#7be4df]' : 'text-[var(--mutqan-accent)]')}>
                {eyebrow}
            </motion.p>
            <motion.h2
                variants={rise}
...
function PaperTexture() {
    return (
        <div className="pointer-events-none absolute inset-0 opacity-[0.34] [background-image:linear-gradient(rgba(11,19,32,0.055)_1px,transparent_1px),linear-gradient(90deg,rgba(11,19,32,0.045)_1px,transparent_1px)] [background-size:38px_38px]" />
    )
}
