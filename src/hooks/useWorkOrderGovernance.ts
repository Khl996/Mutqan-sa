...
export interface GovernanceDecisionQueueItem {
...
export function useWorkOrderGovernance(workOrderId: string, enabled: boolean) {
    ...
    const governanceQuery = useQuery({
        ...
        queryFn: async () => {
            // Generated database types predate the governance tables in this reviewed migration set.
    ...
    const decisionQuery = useQuery({
        ...
        queryFn: async () => {
            // Generated RPC types predate the governance functions in this reviewed migration set.
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const { data, error } = await (supabase as any).rpc('get_governance_decision_queue')
            if (error) throw error

            const rows = Array.isArray(data) ? data as GovernanceDecisionQueueItem[] : []
            return rows.find(item => item.work_order_id === workOrderId) ?? null
        },
    ...
    return {
        ...
        canCurrentActorDecide: isWaitingForManualApproval && !!decisionQuery.data,
