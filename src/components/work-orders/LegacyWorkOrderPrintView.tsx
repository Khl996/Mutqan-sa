...
import type { WorkOrder, OperationLog } from '@/hooks/useWorkOrders'
import { useTenant } from '@/contexts/TenantContext'
import { STATUS_DISPLAY } from '@/config/workOrderStatus'

interface LegacyWorkOrderPrintViewProps {
    workOrder: WorkOrder
    logs: OperationLog[]
}
...
function readPrintIdentity(settings: Record<string, unknown> | null | undefined): PrintIdentity {
    ...
    return typeof pdfIdentity === 'object' && pdfIdentity !== null
        ? pdfIdentity as PrintIdentity
        : {}
}
...
function InfoRow({ label, value, strong = false }: { label: string; value: ReactNode; strong?: boolean }) {
    return (
        <tr className="border-b border-slate-100">
            ...
            <td className={strong ? 'py-2 font-bold text-slate-950' : 'py-2 text-slate-900'}>
                {value || '-'}
            </td>
        </tr>
    )
...
const LegacyWorkOrderPrintView = forwardRef<HTMLDivElement, LegacyWorkOrderPrintViewProps>(({
    workOrder,
    logs,
}, ref) => {
        ...
        >
            ...
            <section className="mb-7">
                ...
                <table className="w-full border-collapse text-sm">
                    ...
                    <tbody>
                        {logs.map((log, index) => (
                            <tr key={log.id || index} className={`print-table-row ${index % 2 === 0 ? 'bg-white' : 'bg-slate-50'}`}>
            ...
            <section className="print-avoid-break mt-14 grid grid-cols-2 gap-x-10 gap-y-12 text-center text-sm">
                {['توقيع الفني', 'توقيع المشرف', 'توقيع مدير الموقع / المهندس', 'توقيع مدير الصيانة'].map((label) => (
                    <div key={label}>
...
LegacyWorkOrderPrintView.displayName = 'LegacyWorkOrderPrintView'

export default LegacyWorkOrderPrintView
