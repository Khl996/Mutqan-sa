-- =============================================================================
-- Migration: 114_phase2_reporting_foundation_metrics.sql
-- Purpose:
--   Establish a small aggregate RPC for future ROI/SLA/cost reporting.
--
-- Notes:
--   - This does not add a new dashboard or product surface.
--   - It prevents future reports from pulling raw operational rows into the UI.
--   - The function enforces tenant scope internally because it is SECURITY DEFINER.
-- =============================================================================
...
BEGIN
    ...
    inventory_consumption AS (
        SELECT
            COALESCE(SUM(ABS(quantity)), 0)::numeric AS consumed_quantity_30d,
            COALESCE(SUM(ABS(COALESCE(total_cost, quantity * COALESCE(unit_cost, 0)))), 0)::numeric AS consumed_value_30d
