-- Fix profile status column and recreate function
-- 1. Ensure `status` column exists in `profiles` (it seems it was missing or named differently)
-- Based on error "column 'status' of relation 'profiles' does not exist"
-- We will check if `is_active` is used instead or if we need to add `status`.
-- The types file shows `is_active` boolean, but the function uses `status`.
-- Let's stick to `is_active` boolean as it is more standard and matches the types.

ALTER TABLE profiles 
DROP COLUMN IF EXISTS status; -- Drop if it exists to avoid confusion, or ignore if we want to migrate.

-- 2. Update the register_new_tenant function to use `is_active` instead of `status`
