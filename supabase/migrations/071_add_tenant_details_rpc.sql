...
-- Drop the old function first to avoid signature conflicts if we just replace
DROP FUNCTION IF EXISTS register_new_tenant;
...
BEGIN
    ...
    INSERT INTO tenants (
        ...
        name_ar, 
        slug,
        -- status removed (doesn't exist)
        plan_id, -- Correct column name
        subscription_status,
        cr_number,
        tax_number,
        address,
