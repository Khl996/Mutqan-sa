...
--
--   Non-destructive:
--     - Fixes Wave-1 anonymous table exposure first.
--     - Extends work_order_governance with pending_approval and matrix metadata.
--     - Adds tenant-scoped approval_matrix_rules.
--     - Adds audited approval evaluation/decision RPCs.
--     - Adds a default-allow start guard: only explicit pending/rejected
--       standard governance rows block start.
-- =============================================================================

-- -----------------------------------------------------------------------------
-- 0. SECURITY FIRST: Wave-1 governance tables must not be exposed to anon.
-- -----------------------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS public.approval_matrix_rules (
    ...
    approval_tier TEXT NOT NULL CHECK (
        approval_tier IN (
            'auto',
            'routine',
            'supervisor',
    ...
    escalation_role TEXT NOT NULL DEFAULT 'maintenance_manager' CHECK (
        escalation_role IN (
            'supervisor',
            'engineer',
            'facility_manager',
    ...
    CONSTRAINT approval_matrix_rules_amount_range_check CHECK (
        amount_min IS NULL
...
CREATE INDEX IF NOT EXISTS idx_approval_matrix_rules_tenant_active
    ON public.approval_matrix_rules(tenant_id, is_active, priority);

CREATE INDEX IF NOT EXISTS idx_approval_matrix_rules_thresholds
    ON public.approval_matrix_rules(tenant_id, severity_min_rank, criticality_min_rank, amount_min, amount_max)
    WHERE is_active;

COMMENT ON TABLE public.approval_matrix_rules IS
    'Tenant-scoped Field Governance approval matrix rules. Wave 2.2 uses work_orders.estimated_cost as the trusted amount source.';
...
CREATE POLICY "approval_matrix_rules_select_scoped"
...
-- -----------------------------------------------------------------------------
-- 2. Extend the existing Wave-1 governance state layer.
-- -----------------------------------------------------------------------------

ALTER TABLE public.work_order_governance
    DROP CONSTRAINT IF EXISTS work_order_governance_governance_state_check;

ALTER TABLE public.work_order_governance
    ADD CONSTRAINT work_order_governance_governance_state_check CHECK (
        governance_state IN (
            'standard',
            'pending_approval',
...
BEGIN
    IF NOT EXISTS (
        SELECT 1
          FROM pg_constraint
         WHERE conname = 'work_order_governance_approval_rule_id_fkey'
           AND conrelid = 'public.work_order_governance'::regclass
    ) THEN
        ALTER TABLE public.work_order_governance
            ADD CONSTRAINT work_order_governance_approval_rule_id_fkey
            FOREIGN KEY (approval_rule_id)
            REFERENCES public.approval_matrix_rules(id)
            ON DELETE SET NULL;
    END IF;
END $$;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1
          FROM pg_constraint
         WHERE conname = 'work_order_governance_approval_amount_source_check'
           AND conrelid = 'public.work_order_governance'::regclass
    ) THEN
        ALTER TABLE public.work_order_governance
            ADD CONSTRAINT work_order_governance_approval_amount_source_check
            CHECK (
                approval_amount_source IS NULL
                OR approval_amount_source IN ('estimated_cost', 'none')
            );
    END IF;
    ...
    IF NOT EXISTS (
        SELECT 1
          FROM pg_constraint
         WHERE conname = 'work_order_governance_approval_roles_check'
           AND conrelid = 'public.work_order_governance'::regclass
    ) THEN
        ALTER TABLE public.work_order_governance
            ADD CONSTRAINT work_order_governance_approval_roles_check
            CHECK (
                (requested_approver_role IS NULL OR requested_approver_role IN ('supervisor', 'engineer', 'facility_manager', 'maintenance_manager', 'tenant_admin'))
                AND (required_approver_role IS NULL OR required_approver_role IN ('supervisor', 'engineer', 'facility_manager', 'maintenance_manager', 'tenant_admin'))
                AND (escalated_from_role IS NULL OR escalated_from_role IN ('supervisor', 'engineer', 'facility_manager', 'maintenance_manager', 'tenant_admin'))
            );
    END IF;
END $$;

CREATE INDEX IF NOT EXISTS idx_work_order_governance_approval_rule
    ON public.work_order_governance(approval_rule_id);

CREATE INDEX IF NOT EXISTS idx_work_order_governance_decision_queue
    ON public.work_order_governance(tenant_id, required_approver_role, approval_due_at)
    WHERE governance_state IN ('pending_approval', 'post_action_complete');

COMMENT ON COLUMN public.work_order_governance.governance_state IS
    'Governance sub-state. Wave 2.2 adds pending_approval for standard pre-execution approval. Direct changes remain blocked by trigger unless an audited governance RPC sets app.governance_workflow_authorized.';

COMMENT ON COLUMN public.work_order_governance.approval_amount_source IS
    'Wave 2.2 amount source. Only work_orders.estimated_cost is trusted in this wave; actual_cost is deliberately not used for pre-execution approval.';

-- -----------------------------------------------------------------------------
-- 3. Internal helpers.
-- -----------------------------------------------------------------------------

CREATE OR REPLACE FUNCTION public.governance_severity_rank(p_severity TEXT)
RETURNS INTEGER
LANGUAGE sql
IMMUTABLE
AS $$
    SELECT CASE COALESCE(BTRIM(p_severity), '')
        WHEN 'life_safety' THEN 5
...
CREATE OR REPLACE FUNCTION public.governance_criticality_rank(p_criticality TEXT)
RETURNS INTEGER
LANGUAGE sql
IMMUTABLE
AS $$
    SELECT CASE COALESCE(BTRIM(p_criticality), '')
        WHEN 'critical' THEN 4
        WHEN 'high' THEN 3
        WHEN 'medium' THEN 2
        WHEN 'low' THEN 1
...
CREATE OR REPLACE FUNCTION public.governance_role_rank(p_role TEXT)
RETURNS INTEGER
LANGUAGE sql
IMMUTABLE
AS $$
    SELECT CASE COALESCE(BTRIM(p_role), '')
        WHEN 'supervisor' THEN 2
        WHEN 'engineer' THEN 3
        WHEN 'facility_manager' THEN 4
        WHEN 'maintenance_manager' THEN 4
        WHEN 'tenant_admin' THEN 5
        WHEN 'platform_admin' THEN 6
        WHEN 'platform_owner' THEN 7
        ELSE 0
    END;
$$;

CREATE OR REPLACE FUNCTION public.governance_next_escalation_role(p_role TEXT)
RETURNS TEXT
LANGUAGE sql
IMMUTABLE
AS $$
    SELECT CASE COALESCE(BTRIM(p_role), '')
        WHEN 'supervisor' THEN 'maintenance_manager'
        WHEN 'engineer' THEN 'maintenance_manager'
        WHEN 'facility_manager' THEN 'maintenance_manager'
        WHEN 'maintenance_manager' THEN 'tenant_admin'
        WHEN 'tenant_admin' THEN 'tenant_admin'
        ELSE 'maintenance_manager'
    END;
$$;

CREATE OR REPLACE FUNCTION public.governance_actor_can_decide(
    p_actor_role TEXT,
...
IMMUTABLE
AS $$
    SELECT COALESCE(p_is_super_admin, FALSE)
        OR p_actor_role IN ('platform_owner', 'platform_admin')
        OR p_actor_role = 'tenant_admin'
        OR p_actor_role = p_required_role;
$$;

CREATE OR REPLACE FUNCTION public.governance_has_active_approver(
    p_tenant_id UUID,
    p_role TEXT
)
RETURNS BOOLEAN
    ...
    LOOP
        ...
        IF public.governance_has_active_approver(p_tenant_id, v_next) THEN
            RETURN jsonb_build_object(
                ...
                'requested_role', v_role,
                'escalated', TRUE,
                'reason', 'no_active_approver_for_' || v_role
            );
        END IF;
...
CREATE OR REPLACE FUNCTION public.ensure_default_approval_matrix_rules(p_tenant_id UUID)
RETURNS VOID
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
    IF p_tenant_id IS NULL THEN
        RAISE EXCEPTION 'Tenant id is required for default approval matrix rules'
            USING ERRCODE = '23502';
    END IF;
    ...
    VALUES
        (
            p_tenant_id,
            'life_safety_top',
            'Life-safety override always maps to the top approval tier.',
            5,
            1,
            NULL,
            NULL,
        ...
        (
            p_tenant_id,
            'critical_asset',
            'Any work on a critical asset requires maintenance manager approval.',
            1,
            4,
            NULL,
            NULL,
        ...
        (
            p_tenant_id,
            'high_amount',
            'High estimated cost requires maintenance manager approval.',
            1,
            1,
            10000,
            NULL,
        ...
        (
            p_tenant_id,
            'high_severity',
            'High severity routine work requires supervisor approval.',
            3,
            1,
            NULL,
            NULL,
        ...
        (
            p_tenant_id,
            'routine_auto',
            'Routine work below explicit escalation thresholds is auto-approved.',
            1,
            1,
            NULL,
            9999.99,
...
BEGIN
    ...
    SELECT *
     ...
     WHERE r.tenant_id = p_tenant_id
       ...
       AND (r.amount_min IS NULL OR v_amount >= r.amount_min)
       AND (r.amount_max IS NULL OR v_amount <= r.amount_max)
     ORDER BY r.priority ASC, r.created_at ASC
     LIMIT 1;

    IF NOT FOUND THEN
        RAISE EXCEPTION 'No active approval matrix rule matched tenant %, severity rank %, criticality rank %, amount %',
...
-- -----------------------------------------------------------------------------
-- 4. Approval evaluation RPC. Auto-approval uses the same approved transition
--    audit shape as manual approval: governance.approved with before/after.
-- -----------------------------------------------------------------------------
    ...
    IF v_route_type = 'emergency_override' THEN
        ...
        IF v_escalated THEN
            PERFORM public.create_governance_log_event(
                v_wo.tenant_id,
                v_wo.id,
                'Governance approval escalated because the requested approver role has no active approver',
                v_escalation_reason,
                v_actor_id,
...
-- -----------------------------------------------------------------------------
-- 5. Standard governance decision RPCs. These use the same governance_state
--    trigger/audit helper that Wave 1 uses for emergency decisions.
-- -----------------------------------------------------------------------------
...
BEGIN
    ...
    IF NOT public.governance_actor_can_decide(v_actor_role, v_governance.required_approver_role, v_actor_super) THEN
        RAISE EXCEPTION 'Unauthorized: required approver role is %, your role is %',
            v_governance.required_approver_role, v_actor_role
            USING ERRCODE = '42501';
    END IF;
...
-- -----------------------------------------------------------------------------
-- 6. Minimal role-scoped decision queue RPC. It is a filter over
--    work_order_governance, not a state store.
-- -----------------------------------------------------------------------------
...
BEGIN
    ...
    IF NOT COALESCE(v_actor_active, FALSE) THEN
        RAISE EXCEPTION 'Inactive users cannot read governance decision queue'
            USING ERRCODE = '42501';
    END IF;
    ...
    SELECT
      ...
      JOIN public.work_orders w ON w.id = g.work_order_id
     WHERE (
            (v_actor_super OR v_actor_role IN ('platform_owner', 'platform_admin'))
            OR (
                w.tenant_id = v_actor_tenant
                AND g.required_approver_role = v_actor_role
            )
        )
       AND (
            g.governance_state = 'pending_approval'
            OR (
                g.route_type = 'emergency_override'
                AND g.governance_state = 'post_action_complete'
            )
        )
     ORDER BY g.approval_due_at NULLS LAST, g.created_at ASC;
END;
$$;
...
-- -----------------------------------------------------------------------------
-- 7. Default-allow start guard. Legacy work orders with no governance row
--    start normally. Only explicit standard pending/rejected rows are blocked.
-- -----------------------------------------------------------------------------
...
BEGIN
    ...
    IF v_governance_state = 'pending_approval' THEN
        RAISE EXCEPTION 'Standard work order cannot start while governance approval is pending; governance_state=pending_approval'
            USING ERRCODE = '42501';
    END IF;

    IF v_governance_state = 'rejected' THEN
        RAISE EXCEPTION 'Standard work order cannot start after governance rejection; governance_state=rejected'
            USING ERRCODE = '42501';
    END IF;
...
DROP TRIGGER IF EXISTS trg_guard_standard_governance_before_work_order_start
    ON public.work_orders;

CREATE TRIGGER trg_guard_standard_governance_before_work_order_start
BEFORE UPDATE OF status ON public.work_orders
FOR EACH ROW
EXECUTE FUNCTION public.guard_standard_governance_before_work_order_start();

COMMENT ON FUNCTION public.guard_standard_governance_before_work_order_start() IS
    'Wave 2.2 default-allow start guard. Blocks start only for explicit standard governance rows in pending_approval or rejected; legacy work orders with no governance row start normally.';

-- -----------------------------------------------------------------------------
-- 8. Emergency rejection closure: rejected emergency work can be closed as a
--    rejected governance exception, never as approved/completed.
-- -----------------------------------------------------------------------------

CREATE OR REPLACE FUNCTION public.close_rejected_emergency_work_order(
    p_work_order_id UUID,
    p_notes TEXT
)
RETURNS JSONB
...
DECLARE
    ...
    v_is_platform BOOLEAN := FALSE;
    v_wo public.work_orders%ROWTYPE;
    v_before_wo public.work_orders%ROWTYPE;
    v_governance public.work_order_governance%ROWTYPE;
BEGIN
    IF v_actor_id IS NULL THEN
        RAISE EXCEPTION 'Authentication required' USING ERRCODE = '28000';
    ...
    IF NOT COALESCE(v_actor_active, FALSE) THEN
        RAISE EXCEPTION 'Inactive users cannot close rejected emergency governance'
            USING ERRCODE = '42501';
    END IF;
    ...
    IF v_actor_role NOT IN ('tenant_admin', 'maintenance_manager', 'platform_owner', 'platform_admin')
       AND NOT v_actor_super THEN
        RAISE EXCEPTION 'Unauthorized: your role is not permitted to close rejected emergency governance'
            USING ERRCODE = '42501';
    END IF;
    ...
    IF v_governance.governance_state <> 'rejected' THEN
        RAISE EXCEPTION 'Cannot close rejected emergency exception from governance_state: %',
            v_governance.governance_state
            USING ERRCODE = '22023';
    END IF;
    ...
    PERFORM public.create_governance_log_event(
        v_wo.tenant_id,
        v_wo.id,
        'Rejected emergency governance exception closed',
        NULLIF(BTRIM(p_notes), ''),
        v_actor_id,
        'governance.rejected_exception_closed',
        'work_order_governance',
        v_governance.id,
        jsonb_build_object(
            'governance_state', v_governance.governance_state,
            'work_order_status', v_before_wo.status
        ),
        jsonb_build_object(
            'governance_state', v_governance.governance_state,
            'work_order_status', v_wo.status
        ),
        jsonb_build_object(
            'notes', p_notes,
            'route_type', v_governance.route_type
        )
    );
...
REVOKE EXECUTE ON FUNCTION public.close_rejected_emergency_work_order(UUID, TEXT) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.close_rejected_emergency_work_order(UUID, TEXT) FROM anon;
GRANT EXECUTE ON FUNCTION public.close_rejected_emergency_work_order(UUID, TEXT) TO authenticated;

-- -----------------------------------------------------------------------------
-- 9. Tighten existing emergency decision RPCs when a matrix role is present.
...
BEGIN
    ...
    IF v_governance.required_approver_role IS NOT NULL
       AND NOT public.governance_actor_can_decide(v_actor_role, v_governance.required_approver_role, v_actor_super) THEN
        RAISE EXCEPTION 'Unauthorized: required approver role is %, your role is %',
            v_governance.required_approver_role, v_actor_role
            USING ERRCODE = '42501';
    END IF;
    ...
    IF v_actor_role NOT IN (
        'tenant_admin', 'maintenance_manager', 'engineer', 'supervisor',
        'platform_owner', 'platform_admin'
    ) AND NOT v_actor_super THEN
        RAISE EXCEPTION 'Unauthorized: your role is not permitted to approve emergency governance'
            USING ERRCODE = '42501';
    END IF;
...
BEGIN
    ...
    IF v_actor_role NOT IN (
        ...
        RAISE EXCEPTION 'Unauthorized: your role is not permitted to reject emergency governance'
            USING ERRCODE = '42501';
    END IF;
...
COMMENT ON FUNCTION public.evaluate_work_order_approval(UUID) IS
    'Wave 2.2 approval matrix evaluator. Standard auto-approval transitions pending_approval -> approved through the same governance_state audit trigger used by manual approval.';

COMMENT ON FUNCTION public.approve_governance_decision(UUID, TEXT) IS
    'Approves a standard pending_approval governance decision. State transition audit is automatic via Wave-1 trigger/helper.';

COMMENT ON FUNCTION public.reject_governance_decision(UUID, TEXT) IS
    'Rejects a standard pending_approval governance decision. State transition audit is automatic via Wave-1 trigger/helper.';

COMMENT ON FUNCTION public.get_governance_decision_queue() IS
    'Role-scoped decision queue filter over work_order_governance. Stores no queue state.';
