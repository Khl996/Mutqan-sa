-- =============================================================================
-- Migration: 131_field_governance_wave1.sql
-- Purpose:
--   Phase 2 / Wave 1 Field Governance foundation.
--
--   Non-destructive:
--     - Adds work_order_governance as a state layer above work_orders.
--     - Adds operation_log_events as an append-only detail layer linked to
--       operation_logs via ON DELETE RESTRICT.
--     - Adds governance operation log type.
--     - Adds emergency and post-action RPCs without changing existing
--       start_work_order / wo_start signatures.
-- =============================================================================

-- -----------------------------------------------------------------------------
-- 1. Extend operation_logs.type for governance parent log rows.
-- -----------------------------------------------------------------------------
...
COMMENT ON CONSTRAINT operation_logs_type_check ON public.operation_logs IS
    'Allowed operation log types. governance added by migration 131 for Field Governance Wave 1.';

-- -----------------------------------------------------------------------------
-- 2. Governance state layer.
-- -----------------------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS public.work_order_governance (
    ...
    route_type TEXT NOT NULL DEFAULT 'standard'
        CHECK (route_type IN ('standard', 'emergency_override')),
    ...
    override_severity TEXT
        CHECK (
            override_severity IS NULL
            OR override_severity IN ('low', 'medium', 'high', 'critical', 'life_safety')
        ),
    ...
    CONSTRAINT work_order_governance_emergency_minimum_check CHECK (
        ...
        OR (
            ...
            AND override_evidence <> '{}'::JSONB
            AND (
                affected_asset_id IS NOT NULL
                OR affected_building_id IS NOT NULL
                OR affected_floor_id IS NOT NULL
                OR affected_room_id IS NOT NULL
            )
        )
    )
);

CREATE INDEX IF NOT EXISTS idx_work_order_governance_tenant
    ON public.work_order_governance(tenant_id);

CREATE INDEX IF NOT EXISTS idx_work_order_governance_state
    ON public.work_order_governance(governance_state);

CREATE INDEX IF NOT EXISTS idx_work_order_governance_route
    ON public.work_order_governance(route_type);

CREATE INDEX IF NOT EXISTS idx_work_order_governance_affected_asset
    ON public.work_order_governance(affected_asset_id);

CREATE INDEX IF NOT EXISTS idx_work_order_governance_affected_building
    ON public.work_order_governance(affected_building_id);

CREATE INDEX IF NOT EXISTS idx_work_order_governance_affected_floor
    ON public.work_order_governance(affected_floor_id);

CREATE INDEX IF NOT EXISTS idx_work_order_governance_affected_room
    ON public.work_order_governance(affected_room_id);

CREATE INDEX IF NOT EXISTS idx_work_order_governance_started_by
    ON public.work_order_governance(started_by);

CREATE INDEX IF NOT EXISTS idx_work_order_governance_post_action_completed_by
    ON public.work_order_governance(post_action_completed_by);

CREATE INDEX IF NOT EXISTS idx_work_order_governance_decision_by
    ON public.work_order_governance(decision_by);

COMMENT ON TABLE public.work_order_governance IS
    'Field Governance state layer above work_orders. Does not add or replace work_orders.status.';

COMMENT ON COLUMN public.work_order_governance.governance_state IS
    'Governance sub-state. Direct changes are blocked by trigger unless an audited governance RPC sets app.governance_workflow_authorized.';

-- -----------------------------------------------------------------------------
-- 3. Append-only operation log event layer.
-- -----------------------------------------------------------------------------
...
CREATE INDEX IF NOT EXISTS idx_operation_log_events_tenant
    ON public.operation_log_events(tenant_id);

CREATE INDEX IF NOT EXISTS idx_operation_log_events_operation_log
    ON public.operation_log_events(operation_log_id);

CREATE INDEX IF NOT EXISTS idx_operation_log_events_work_order
    ON public.operation_log_events(work_order_id);

CREATE INDEX IF NOT EXISTS idx_operation_log_events_type
    ON public.operation_log_events(event_type);

CREATE INDEX IF NOT EXISTS idx_operation_log_events_actor
    ON public.operation_log_events(actor_id);

COMMENT ON TABLE public.operation_log_events IS
    'Append-only governance event rows linked to operation_logs via ON DELETE RESTRICT.';
...
-- -----------------------------------------------------------------------------
-- 5. Append-only and governance-state guard triggers.
-- -----------------------------------------------------------------------------
...
DROP TRIGGER IF EXISTS trg_guard_operation_log_events_append_only
    ON public.operation_log_events;
...
BEGIN
    ...
    v_authorized := COALESCE(
        current_setting('app.governance_workflow_authorized', TRUE) = 'true',
        FALSE
    );

    IF NOT v_authorized THEN
        RAISE EXCEPTION
            'Direct update of governance_state is not permitted. Use audited governance RPCs. Field Governance Wave 1.'
            USING ERRCODE = '42501';
    END IF;
...
DROP TRIGGER IF EXISTS trg_guard_work_order_governance_state_update
    ON public.work_order_governance;
...
BEGIN
    IF p_tenant_id IS NULL THEN
        RAISE EXCEPTION 'Tenant id is required for governance audit event'
            USING ERRCODE = '23502';
    END IF;

    IF p_event_type IS NULL OR p_event_type NOT LIKE 'governance.%' THEN
        RAISE EXCEPTION 'Governance event_type must start with governance. (got: %)', p_event_type
            USING ERRCODE = '22023';
    END IF;

    IF p_entity_type IS NULL OR BTRIM(p_entity_type) = '' THEN
        RAISE EXCEPTION 'Governance audit entity_type is required'
            USING ERRCODE = '23502';
    END IF;
    ...
    IF p_work_order_id IS NOT NULL THEN
        ...
        IF NOT FOUND THEN
            RAISE EXCEPTION 'Work order not found for governance audit event'
                USING ERRCODE = 'P0002';
        END IF;
    END IF;

    v_code := 'GOV-' || to_char(NOW(), 'YYMMDDHH24MISS') || '-' ||
...
REVOKE EXECUTE ON FUNCTION public.create_governance_log_event(
    UUID, UUID, TEXT, TEXT, UUID, TEXT, TEXT, UUID, JSONB, JSONB, JSONB
) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.create_governance_log_event(
    UUID, UUID, TEXT, TEXT, UUID, TEXT, TEXT, UUID, JSONB, JSONB, JSONB
) FROM anon;
...
COMMENT ON FUNCTION public.create_governance_log_event(
    UUID, UUID, TEXT, TEXT, UUID, TEXT, TEXT, UUID, JSONB, JSONB, JSONB
) IS
    'Internal Field Governance audit helper. Writes operation_logs parent row plus append-only operation_log_events row.';

-- Automatically audit every authorized governance_state transition.
...
BEGIN
    ...
    v_event_type := COALESCE(
        NULLIF(current_setting('app.governance_event_type', TRUE), ''),
        'governance.state_changed'
    );

    v_description := COALESCE(
        NULLIF(current_setting('app.governance_event_description', TRUE), ''),
        'Governance state changed from ' || OLD.governance_state || ' to ' || NEW.governance_state
    );

    v_context := COALESCE(
        NULLIF(current_setting('app.governance_event_context', TRUE), '')::JSONB,
        '{}'::JSONB
...
DROP TRIGGER IF EXISTS trg_audit_work_order_governance_state_change
    ON public.work_order_governance;
...
-- -----------------------------------------------------------------------------
-- 7. Prevent silent closure of unresolved emergency override work orders.
-- -----------------------------------------------------------------------------
...
BEGIN
    ...
    IF FOUND AND v_governance_state <> 'approved' THEN
        RAISE EXCEPTION
            'Emergency override work order cannot be closed before governance approval; current governance_state=%',
            v_governance_state
            USING ERRCODE = '42501';
    END IF;
...
DROP TRIGGER IF EXISTS trg_guard_emergency_governance_before_work_order_close
...
-- -----------------------------------------------------------------------------
-- 8. Emergency and governance workflow RPCs.
-- -----------------------------------------------------------------------------
...
BEGIN
    ...
    IF p_evidence IS NULL
       ...
       OR p_evidence = '{}'::JSONB
    THEN
        RAISE EXCEPTION 'Emergency override evidence is required'
            USING ERRCODE = '23502';
    END IF;
    ...
    IF NOT COALESCE(v_actor_active, FALSE) THEN
        RAISE EXCEPTION 'Inactive users cannot start emergency work orders'
            USING ERRCODE = '42501';
    END IF;
    ...
    IF NOT (v_actor_super OR v_actor_role IN ('platform_owner', 'platform_admin'))
       AND v_actor_tenant IS DISTINCT FROM v_wo.tenant_id
    THEN
    ...
    -- Route through the existing live start path. This preserves the 120 guard
    -- pattern and keeps start_work_order(UUID) unchanged.
    ...
    PERFORM public.create_governance_log_event(
        ...
        jsonb_build_object(
            ...
            'override_severity', v_governance.override_severity
        )
    );
...
REVOKE EXECUTE ON FUNCTION public.start_work_order_emergency(
...
COMMENT ON FUNCTION public.start_work_order_emergency(
    UUID, TEXT, TEXT, JSONB, UUID, UUID, UUID, UUID
) IS
    'Starts an emergency override through existing start_work_order(UUID), then creates governance state and audit event.';
...
BEGIN
    ...
    IF COALESCE(BTRIM(p_temp_action), '') = '' THEN
        RAISE EXCEPTION 'Temporary action is required for emergency post-action completion'
            USING ERRCODE = '23502';
    END IF;

    IF COALESCE(BTRIM(p_verbal_approver_name), '') = ''
       AND p_verbal_approver_profile_id IS NULL
    THEN
        RAISE EXCEPTION 'Verbal approver is required for emergency post-action completion'
            USING ERRCODE = '23502';
    END IF;

    IF p_post_action_deadline IS NULL THEN
        RAISE EXCEPTION 'Post-action approval deadline is required'
            USING ERRCODE = '23502';
    END IF;
    ...
    IF NOT COALESCE(v_actor_active, FALSE) THEN
        RAISE EXCEPTION 'Inactive users cannot complete emergency post-action fields'
            USING ERRCODE = '42501';
    END IF;
    ...
    IF v_actor_role NOT IN (
        ...
        RAISE EXCEPTION 'Unauthorized: your role is not permitted to complete emergency post-action fields'
            USING ERRCODE = '42501';
    END IF;

    IF NOT v_is_management AND v_wo.assigned_to IS DISTINCT FROM v_actor_id THEN
        RAISE EXCEPTION 'Only the assigned technician or management can complete emergency post-action fields'
            USING ERRCODE = '42501';
    END IF;
    ...
    IF v_governance.governance_state <> 'post_action_required' THEN
        RAISE EXCEPTION 'Cannot complete post-action fields from governance_state: %',
    ...
    UPDATE public.work_order_governance
       SET governance_state = 'post_action_complete',
           temp_action = BTRIM(p_temp_action),
...
REVOKE EXECUTE ON FUNCTION public.complete_emergency_post_action(
    UUID, TEXT, TEXT, UUID, TIMESTAMPTZ, JSONB
) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.complete_emergency_post_action(
    UUID, TEXT, TEXT, UUID, TIMESTAMPTZ, JSONB
) FROM anon;
GRANT EXECUTE ON FUNCTION public.complete_emergency_post_action(
    UUID, TEXT, TEXT, UUID, TIMESTAMPTZ, JSONB
) TO authenticated;
...
BEGIN
    ...
    PERFORM set_config(
        'app.governance_event_context',
        jsonb_build_object('notes', p_notes)::TEXT,
        TRUE
    );
...
COMMENT ON FUNCTION public.complete_emergency_post_action(
    UUID, TEXT, TEXT, UUID, TIMESTAMPTZ, JSONB
) IS
    'Completes required emergency post-action fields and moves governance_state to post_action_complete. State transition audit is automatic via trigger.';

COMMENT ON FUNCTION public.approve_emergency_governance(UUID, TEXT) IS
    'Approves completed emergency governance. State transition audit is automatic via trigger.';

COMMENT ON FUNCTION public.reject_emergency_governance(UUID, TEXT) IS
    'Rejects completed emergency governance with required reason. State transition audit is automatic via trigger.';
